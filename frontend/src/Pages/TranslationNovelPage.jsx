...
const TranslationNovelPage = () => {
  ...
  useEffect(() => {
    const fetchNovelData = async () => {
      try {
        ...
        // Fetch novel details
        const novelResponse = await axios.get(`${BASE_URL}/api/translation_novel/${id}`);
        if (novelResponse.data.success) {
          setNovel(novelResponse.data.data);

          // Fetch chapters via GET with query param
          const chaptersResponse = await axios.get(`${BASE_URL}/api/translation_novel/${id}/chapter`, {
            params: { novel_id: id }
          });
        ...
        setError('Error loading translation data. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleReadFirstChapter = () => {
    if (chapters.length > 0) {
      ...
      navigate(`/translation-novel/${novel.id}/chapter/${chapters[0].id}`);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-3 text-indigo-700">Loading translation...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center max-w-md p-6 bg-white rounded-lg shadow-md">
          ...
          <p className="text-gray-600">{error || "Translation not found"}</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <Link to="/translations" className="flex items-center text-indigo-600 hover:text-indigo-800">
          <ArrowLeft className="h-4 w-4 mr-1" /> Back to Translations
        </Link>
      </div>
                      ...
                      {chapters.map((chapter) => (
                        <tr key={chapter.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <Link to={`/translation-novel/${novel.id}/chapter/${chapter.id}`} className="text-indigo-600 hover:text-indigo-900">
                              {chapter.title}
                            </Link>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
