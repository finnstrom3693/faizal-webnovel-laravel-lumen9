...
import { fetchSearchResults, clearSearchResults } from '../redux/actions/searchActions_Old';
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
      <Link to={`/novel/${novel.id}`} className="block">
        <div className="relative bg-gray-100" style={{ aspectRatio: '2 / 3' }}>
          <img 
            src={novel.coverImage || "/api/placeholder/180/270"} 
            alt={novel.title} 
        ...
        <div className="p-4">
          ...
          <p className="text-gray-600 text-sm mb-3 line-clamp-2">{novel.description || 'No description available'}</p>
          <div className="flex justify-between text-xs text-gray-500">
            <div className="flex items-center">
              <Eye className="h-3 w-3 text-indigo-400 mr-1" />
              <span>{novel.views || 'N/A'}</span>
            </div>
            <div className="flex items-center">
              <Clock className="h-3 w-3 text-indigo-400 mr-1" />
              <span>{novel.lastUpdated || 'N/A'}</span>
            </div>
          </div>
        </div>
      </Link>
